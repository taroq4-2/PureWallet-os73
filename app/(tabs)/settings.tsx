...
export default function SettingsScreen() {
  ...
  const { importFromInbox, loading: smsLoading } = useSmsImport();
  ...
  const handleImportSms = async () => {
    ...
    const result = await importFromInbox();
    if (result !== null) {
      ...
      Alert.alert(
        result.imported > 0 ? "تم الاستيراد ✓" : "لا توجد عمليات جديدة",
        result.imported > 0
          ? `تم استيراد ${result.imported} عملية جديدة من ${result.total} رسالة بنكية.`
          : `فُحصت ${result.total} رسالة بنكية ولم يُعثر على عمليات جديدة.`,
      );
    }
  };
  ...
  return (
    ...
    >
      ...
      {/* Import from SMS inbox */}
      {Platform.OS === "android" && (
        <SettingRow
          colors={colors}
          icon="message-circle"
          iconColor="#10B981"
          label={smsLoading ? "جاري الاستيراد…" : "استيراد من رسائل البنك"}
          sublabel="يقرأ رسائل SMS البنكية ويستورد العمليات تلقائياً"
          onPress={smsLoading ? undefined : handleImportSms}
          disabled={smsLoading}
        />
      )}
...
function SettingRow({ colors, icon, iconColor, label, sublabel, onPress, disabled }: any) {
  return (
    <TouchableOpacity
      style={[styles.settingRow, { backgroundColor: colors.card, borderColor: colors.border, opacity: disabled ? 0.5 : 1 }]}
      onPress={onPress}
      activeOpacity={0.75}
      disabled={disabled}
    >
      ...
      <View style={{ flex: 1 }}>
        ...
        {sublabel ? (
          <Text style={[styles.settingSub, { color: colors.mutedForeground }]}>{sublabel}</Text>
        ) : null}
      </View>
